$(document).ready(function () {
    ...
    Dictionary.prototype = {
        ...
        addSchool() {
            ...
            $("#schadd .confirm").click(function () {
                ...
                _this.params.addPrams.dicType = Number("2")
                _this.params.addPrams.dictionaryId = _this.params.getTablePram.dictionaryId;
                _this.params.addPrams.dicValue = inputs.eq(0).val();
                _this.params.addPrams.dicPosition = inputs.eq(1).val()

                if (!_this.params.addPrams.dicValue) {
                    ysFn.msgBig('请填写字典值！', 0, function () {})
                    return;
                }

                if (!_this.params.addPrams.dicPosition) {
                    ysFn.msgBig('请填写序号！', 0, function () {})
                    return;
                }
        ...
        udietooAdmin() {
            ...
            $("#table-1").on("click", ".blued", function (res) {
                ...
                _this.params.udietoo.id = $(this).parent().parent().attr("data-uniqueid");
                ysAjax.get(_this.params.apis.InvertApi+"/"+ _this.params.udietoo.id,"", "json", function (res) {
                    // console.log(res)
                    inputs.eq(0).val(res.data.dicValue)
                    inputs.eq(1).val(res.data.dicPosition)
                })
                $('#update').ysValidate({
                    tooltip: true,
                    fields: {
            ...
            $("#update .confirm").click(function () {
                ...
                _this.params.udietoo.dicPosition = inputs.eq(1).val();
    ...
    var params = {
        ...
        description: "",
        apis: {
            selectListApi: "/sys/diccode/list", //查询左边列表
